...
export const initiatePayment = async (req, res) => {
  ...
  try {
    ...
    if (!khaltiRes?.pidx || !khaltiRes?.payment_url) {
      throw new ApiError(
        500,
        "Invalid response from Khalti - missing pidx or payment_url",
      );
    }
    ...
    if (error.response?.status === 503 || error.message.includes("503")) {
      return res.status(503).json({
        success: false,
        message:
          "Khalti service is temporarily unavailable. Please try again in a few minutes.",
      });
    }
...
export const paymentTransaction = asyncHandler(async (req, res) => {
  ...
  const amountNPR = Number(fare);

  if (!Number.isFinite(amountNPR)) {
    throw new ApiError(400, "Fare amount is required");
  }

  if (amountNPR <= 0) {
    throw new ApiError(400, "Fare amount must be greater than zero");
  }
  ...
  const payload = {
    ...
    // Khalti API expects paisa.
    amount: Math.round(amountNPR * 100),

    purchase_order_id: `topup_${Date.now()}`,
    purchase_order_name: "NFC Card Top-up",
    customer_info: {
      name: passenger.FirstName || passenger.fullName || "Passenger",
  ...
  const txn = await Transaction.create({
    ...
    // This endpoint creates a Khalti top-up transaction.
    // No trip fare is debited here.
    fare: 0,
    requiredTopup: amountNPR,

    status: "payment_initiated",
    isAutoTopup: true,
    paymentMethod: "khalti_auto",
    description: "Khalti top-up for NFC card",
...
// export const paymentTransaction = asyncHandler(async (req, res) => {
//   const { fare } = req.body;
//   if (!fare) throw new ApiError(400, "Fare amount is required");
//   if (fare <= 0)
//     throw new ApiError(400, "Fare amount must be greater than zero");
//   // if (!fare || fare <= 0)
//   //   throw new ApiError(400, "Valid fare amount is required");
